...
mkdir -p "$SITE_DIR"/{nginx/conf.d,php,mariadb/conf.d,wordpress,logs}

# **2. Sao chép cấu hình NGINX Backend**
...
if [ -f "$NGINX_CONF_TEMPLATE" ]; then
    cp "$NGINX_CONF_TEMPLATE" "$NGINX_CONF_TARGET"
    # Kiểm tra nếu tập tin cấu hình NGINX Backend đã tồn tại trước khi sửa
    if [ -f "$NGINX_CONF_TARGET" ]; then
        # Kiểm tra hệ điều hành để sử dụng `sed` đúng cách
        if [[ "$OSTYPE" == "darwin"* ]]; then
            sed -i '' -e "s|\${SITE_NAME}|$site_name|g" -e "s|\${DOMAIN}|$domain|g" "$NGINX_CONF_TARGET"
        else
            sed -i -e "s|\${SITE_NAME}|$site_name|g" -e "s|\${DOMAIN}|$domain|g" "$NGINX_CONF_TARGET"
        fi
    else
        echo -e "${RED}❌ Lỗi: Tệp tin cấu hình NGINX Backend không tồn tại: $NGINX_CONF_TARGET${NC}"
        exit 1
    fi

    echo -e "${GREEN}✅ Cấu hình NGINX Backend đã được tạo.${NC}"
else
    echo -e "${RED}❌ Lỗi: Không tìm thấy template NGINX Backend.${NC}"
    exit 1
fi

# **2.1. Tạo file cấu hình NGINX Proxy cho tên miền**
...
if is_file_exist "$NGINX_PROXY_CONF_TEMPLATE"; then
    ...
    # Kiểm tra nếu tập tin đã tồn tại và thay thế biến
...
# **3. Sao chép cấu hình PHP-FPM và MariaDB**
