...
if [[ -z "$domain" ]]; then
    echo -e "${RED}${CROSSMARK} Missing environment variable domain. Please export before calling the script.${NC}"
    exit 1
fi
...
if is_file_exist "$NGINX_TEMPLATE"; then
    ...
    cp "$NGINX_TEMPLATE" "$NGINX_CONF" || { echo -e "${RED}${CROSSMARK} Could not copy NGINX template.${NC}"; rm -f "$NGINX_CONF"; exit 1; }
