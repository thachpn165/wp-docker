...
backup_scheduler_create() {
  ...
  if [[ "$storage_choice" == "2" ]]; then
    ...
    while true; do
      rclone_storage=$(get_input_or_test_value "$PROMPT_ENTER_STORAGE_NAME" "${TEST_RCLONE_STORAGE:-demostorage}" )
      rclone_storage=$(echo "$rclone_storage" | xargs)

      if [[ " ${storages[*]} " =~ " ${rclone_storage} " ]]; then
        print_msg success "$SUCCESS_RCLONE_STORAGE_SELECTED: $rclone_storage"
        storage="cloud"
        break
      else
        print_msg warning "$WARNING_INPUT_INVALID"
